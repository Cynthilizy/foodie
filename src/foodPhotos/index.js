import assorted from "./assorted.jpg";
import beef from "./beef.jpg";
import boiledEggs from "./boiledEggs.jpg";
import salad from "./coleslawSalad.jpg";
import friedRice from "./friedRicePortion.jpg";
import iceTea from "./iceTea.jpg";
import jollofRice from "./jollofRicePortion.jpg";
import mackrel from "./mackrelFish.jpg";
import moimoi from "./moimoi.jpg";
import orangeJuice from "./orangeJuice.png";
import chicken from "./pepperChicken.jpg";
import plantain from "./plantain.jpg";
import snail from "./snail.jpg";
import softDrinks from "./softDrinks.png";
import spaghetti from "./spaghetti.jpg";
import steamedRice from "./steamedRicePortion.jpg";
import tilapia from "./tilapiaFish.jpg";
import turkey from "./turkey.jpg";
import water from "./water.png";
import amala from "./amala.jpg";
import eba from "./eba.jpg";
import fufu from "./fufu.jpg";
import pkomo from "./pkomo.jpg";
import poundedYam from "./poundedYam.jpg";
import specialPlate from "./specialPlate.jpg";
import afang from "./afang-soup.jpeg";
import efoRiro from "./efo-riro-soup.jpeg";
import egusi from "./egusi-soup.jpg";
import oha from "./oha-soup.jpg";

export const foodPhotos = {
  assorted,
  beef,
  boiledEggs,
  salad,
  friedRice,
  iceTea,
  jollofRice,
  mackrel,
  moimoi,
  orangeJuice,
  chicken,
  plantain,
  snail,
  softDrinks,
  spaghetti,
  steamedRice,
  tilapia,
  turkey,
  water,
  amala,
  eba,
  fufu,
  pkomo,
  poundedYam,
  specialPlate,
  afang,
  efoRiro,
  egusi,
  oha,
};
